...
$(function(){
	//商品缩略图左右移动效果
	//点击后退
	$('.goods_content').on('click','#backward',function(){
	...
	//点击前进
	$('.goods_content').on('click','#forward',function(){
		var left = parseInt($(".smallpic_wrap ul").css("left")); //获取ul水平方向偏移量
	...
	//选择货品，如颜色、版本等
	$('.goods_content').on('click','.product a',function(){
	...
	//购买数量
	//减少
	$('.goods_content').on('click','#reduce_num',function(){
	...
	//增加
	$('.goods_content').on('click','#add_num',function(){
		$(".amount").val(parseInt($(".amount").val()) + 1);
	})
	...
	//直接输入
	$('.goods_content').on('click','.amount',function(){
		if (parseInt($(".amount").val()) < 1){
			alert("商品数量最少为1");
			$(this).val(1);
		}
	...
	//商品详情效果
	$('.goods_content').on('click','.detail_hd li',function(){
	...
	//图片切换
	$('.goods_content').on('click','.smallpic_wrap li img',function(){
		$(this).parents('.preview').find('.midpic img').attr('src',$(this).attr('src'));
		$(this).parents('.preview').find('.midpic .jqzoom').attr('href',$(this).attr('src'));
	})
});